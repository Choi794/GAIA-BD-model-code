...
# Load the data
rm(list=ls())
data <- fread("E:/GAIA_BD/final_77777777777/Abundance/Abundance2/Abundance_covariate_new12.csv")
...
excluded_vars <- c("NO","hexID", "slope_IC", "NumSpecies", "LAT", "LON", "country", "BM", "geometry")
...
p <- ggplot(cv_plot_data, aes(x = Predicted, y = Actual)) +
  ...
  geom_smooth(method = "lm", se = FALSE, size = 1.2, aes(linetype = "Trendline", color = "Trendline")) +     #orientation = 'y'
...
# Save the plot
jpeg("E:/GAIA_BD/The Biodiversity Knowledge Paradox/figure/RandomForest2/xgboost_abundance_final.jpg", res = 600, width = 10, height = 10, units = "in")
print(p_marginal)
dev.off()
